...
/** Incremental SHA-256 in plain JS: lets us hash multi-hundred-MB videos in slices without holding them in memory. */
export class Sha256 {
  ...
  digestHex(): string {
    ...
    const hi = Math.floor(bits / 0x100000000),
      lo = bits >>> 0;
    for (let i = 0; i < 4; i++) {
      pad[padLen + i] = (hi >>> (24 - 8 * i)) & 0xff;
      pad[padLen + 4 + i] = (lo >>> (24 - 8 * i)) & 0xff;
    }
...
/** Native SHA-256 when the platform module is available (fast), pure JS otherwise. */
export async function sha256Hex(bytes: Uint8Array): Promise<string> {
  try {
    return hex(
      await Crypto.digest(Crypto.CryptoDigestAlgorithm.SHA256, bytes as unknown as ArrayBuffer),
    );
  } catch {
